...
class Visualizer():
    ...
    # save image to the disk
    def save_images2(self, webpage, visuals, image_path, aspect_ratio=1.0):
        image_dir = webpage.get_image_dir()
        short_path = ntpath.basename(image_path[0])
        name = os.path.splitext(short_path)[0]
	...
	#save_path3 = os.path.join(image_dir, 'target/', image_name)
        util.save_image(visuals['real_A'], save_path1)
        util.save_image(visuals['fake_B'], save_path2)
        #util.save_image(visuals['real_B'], save_path3)
